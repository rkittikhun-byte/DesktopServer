...
while ($request = $psr7->waitRequest()) {
    try {
        ...
        $fullPath = $wwwRoot . '/' . ($path ?: 'index.html');
        ...
        if (str_ends_with($fullPath, '.php') && file_exists($fullPath)) {
            ...
            // Send POST body (Pre-fetch for content-length calculation)
            $body = (string)$request->getBody();
            $parsedBody = $request->getParsedBody();
            
            // Reconstruct body if it's already parsed
            ...
            foreach ($request->getHeaders() as $name => $values) {
                if (strtolower($name) === 'accept-encoding') continue; // Let Bridge handle compression
            ...
            // ALWAYS use the actual body length for CONTENT_LENGTH
            if ($request->getMethod() === 'POST' || $body !== '') {
